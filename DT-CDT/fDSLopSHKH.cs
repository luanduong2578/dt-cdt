...
namespace DT_CDT
{
    public partial class fDSLopSHKH : Form
    {
        public fDSLopSHKH()
        {
            InitializeComponent();
        ...
        private void dtgvLH_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (btnLHMoi.Enabled == true)
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    txbLHid.Text = DSLopSKHKDAO.Instance.GetHCKIdByDTTLMASO(txbDTTLMASO.Text).ToString();
        ...
        void LoadLHtoDtgvLH()
        {
            ...
            dtgvLH.DataSource = DSLopSKHKDAO.Instance.LoadLopHoc(namHienTai);
        }

        private void btnLHTim_Click(object sender, EventArgs e)
        {
            dtgvLH.DataSource = DSLopSKHKDAO.Instance.SearchLHbyTuNamDenNam(Convert.ToInt32(cbbSearchLHTuNam.Text), Convert.ToInt32(cbbSearchLHDenNam.Text));
        }
        ...
        {
            dtgvLH.DataSource = DSLopSKHKDAO.Instance.SearchLHbyTenLop(Convert.ToInt32(cbbSearchLHTuNam.Text), Convert.ToInt32(cbbSearchLHDenNam.Text), txbSearchLop.Text);
        }

        private void txbSearchGV_TextChanged(object sender, EventArgs e)
        {
            dtgvLH.DataSource = DSLopSKHKDAO.Instance.SearchLHbyGV(Convert.ToInt32(cbbSearchLHTuNam.Text), Convert.ToInt32(cbbSearchLHDenNam.Text), txbSearchGV.Text);
        ...
        private void btnLHXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                if (DSLopSKHKDAO.Instance.CountHocVienByLop(num))
                {
                ...
                else
                {
                    ...
                    if (drXoa == DialogResult.Yes)
                    {
                        ...
                        DSLopSKHKDAO.Instance.DeleteLopHoc(num);
                        LoadLHtoDtgvLH();
                        LoadButton();
                    }
                }
        ...
        private void btnLHLuu_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (txbLHid.Text == "")
                {
                    Insertbool = DSLopSKHKDAO.Instance.InsertLopHoc(LHNam, TenLop, LHBV, LHKhoaPhong, LHGVTROGIANG, LHDoiTuong, LHThoiGianBatDau, LHThoiGianKetThuc, LHSoTiet, LHSoTietLT, LHSoTietTH, LHDiaDiem, LHKinhPhi, LHChiGV, LHChiNuoc, LHChiKhac, LHGhichu, LOAICHUNGNHAN, idgianvien, LOAICHUNGNHAN);
                    if (Insertbool)
                    {
                        LoadLHtoDtgvLH();
                        LoadButton();
                ...
                {
                    Insertbool = DSLopSKHKDAO.Instance.UpdateLopHoc(LHNam, TenLop, LHBV, LHKhoaPhong, LHGVTROGIANG, LHDoiTuong, LHThoiGianBatDau, LHThoiGianKetThuc, LHSoTiet, LHSoTietLT, LHSoTietTH, LHDiaDiem, LHKinhPhi, LHChiGV, LHChiNuoc, LHChiKhac, LHGhichu, idgianvien, LOAICHUNGNHAN, Convert.ToInt32(txbLHid.Text));
                    if (Insertbool)
                    {
                        LoadLHtoDtgvLH();
                        LoadButton();
