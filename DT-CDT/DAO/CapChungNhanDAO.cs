...
{
    class CapChungNhanDAO
    {
        private static CapChungNhanDAO instance;

        public static CapChungNhanDAO Instance
        {
            get { if (instance == null) instance = new CapChungNhanDAO(); return CapChungNhanDAO.instance; }
            private set { CapChungNhanDAO.instance = value; }
        }
        private CapChungNhanDAO() { }

        public int Count_SOCHUNGNHAN_By_IDLOP_IDHOCVIEN(int IDLOP, int IDHOCVIEN)
        {
            string query = string.Format("SELECT COUNT(SOCHUNGNHAN) from HSOFTDKBD.DT_CAPCHUNGNHAN where IDLOP = {0} and IDHOCVIEN = {1}", IDLOP, IDHOCVIEN);
            int data = Convert.ToInt32(DataProvider.Instance.ExecuteScalar(query));
            return data;
        }

        public string LoadSoCCNDT(int IDLOP, int IDHOCVIEN)
        {
            string data = "";
            if (Count_SOCHUNGNHAN_By_IDLOP_IDHOCVIEN(IDLOP, IDHOCVIEN) > 0)
            {
                string query = string.Format("SELECT SOCHUNGNHAN from HSOFTDKBD.DT_CAPCHUNGNHAN where IDLOP = {0} and IDHOCVIEN = {1}", IDLOP, IDHOCVIEN);
